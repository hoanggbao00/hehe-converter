...
final class VideoPresetStorageTests: XCTestCase {
    func testVideoPresetStorageSeedsRequestedBuiltIns() throws {
        ...
        XCTAssertEqual(presets.first { $0.preset.outputFormat == .webm }?.preset.options?.quality, 33)
        XCTAssertNil(presets.first { $0.preset.outputFormat == .webm }?.preset.options?.fps)
        XCTAssertNil(presets.first { $0.preset.outputFormat == .webm }?.preset.options?.maxWidth)
        XCTAssertEqual(presets.first { $0.preset.outputFormat == .gif }?.preset.options?.fps, 12)
        XCTAssertEqual(presets.first { $0.preset.outputFormat == .gif }?.preset.options?.maxWidth, 375)
        XCTAssertEqual(presets.first { $0.preset.outputFormat == .gif }?.preset.options?.loopCount, 0)
        XCTAssertEqual(presets.first { $0.preset.outputFormat == .webp }?.preset.options?.fps, 12)
        XCTAssertEqual(presets.first { $0.preset.outputFormat == .webp }?.preset.options?.maxWidth, 375)
    ...
    func testVideoPresetCommandsUseExpectedEncoders() throws {
        ...
        XCTAssertTrue(try XCTUnwrap(presets.first { $0.outputFormat == .webm }).ffmpegCommand.contains("-c:v libvpx-vp9 -crf 47 -b:v 0 -c:a libopus"))
        XCTAssertTrue(try XCTUnwrap(presets.first { $0.outputFormat == .gif }).ffmpegCommand.contains("-filter_complex [0:v]fps=12,scale=min(375\\,iw):-2:flags=lanczos,split[v0][v1];[v0]palettegen[p];[v1][p]paletteuse=dither=sierra2_4a:diff_mode=rectangle -loop 0 -an"))
        XCTAssertTrue(try XCTUnwrap(presets.first { $0.outputFormat == .webp }).ffmpegCommand.contains("-vf fps=12,scale=min(375\\,iw):-2:flags=lanczos -an -c:v libwebp -preset picture -lossless 0 -q:v 90 -compression_level 6 -loop 0"))
